...
from dataset.test_dataset_oracle241 import MyDataset, resizeNormalize
...
# Configs
batch_size = 30
resume_path = 'oracle241_content_model.ckpt'
log_dir = '/log_oracle241/'
dataset_dir = '/log_oracle241/prompt_handprint_forscantrain.json'

print(log_dir)

with open('./sort_data_label_train_scan.txt', 'r') as f:
    label = f.readlines()
with open('./sort_data_truelabel_train_scan.txt', 'r') as f:
...
if __name__ == '__main__':
    ...
    for iteration_B, batch in enumerate(dataloader):
        ...
        for i in range(x_samples.shape[0]):
            ...
            true_label_tmp = true_label[label.index(str(batch['label'][i]) + '\n')].strip()
            os.makedirs(root, exist_ok=True)
            filename = "{}_b_{:06}_{}.bmp".format(true_label_tmp, j, str(batch['label'][i]))
